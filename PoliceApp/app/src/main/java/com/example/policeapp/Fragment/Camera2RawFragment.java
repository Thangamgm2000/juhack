package com.example.policeapp.Fragment;
...
import com.example.policeapp.Activity.MainActivity;
import com.example.policeapp.R;
import com.example.policeapp.Views.AutoFitTextureView;
import com.example.policeapp.Views.Manuallpdialog;
...
public class Camera2RawFragment extends Fragment
    ...
    private final CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        public void onCaptureStarted(CameraCaptureSession session, CaptureRequest request,
            ...
            Runnable shuttereffect = new Runnable() {
                @Override
                public void run() {
                    shutter.setVisibility(View.INVISIBLE);
                }
            };
            new Handler().postDelayed(shuttereffect,50);
            String currentDateTime = generateTimestamp();
            File rawFile = new File(getActivity().getExternalFilesDir(null),
                    "RAW"+generateTimestamp()+".dng");
            File jpegFile = new File(getActivity().getExternalFilesDir(null),
                    "JPEG"+generateTimestamp()+".jpg");
            Log.e("check",rawFile.getPath());
...
//            showToast(sb.toString());
    ...
    private static class ImageSaver implements Runnable {
        ...
        private ImageSaver(Image image, File file, CaptureResult result,
                           ...
                           RefCountedAutoCloseable<ImageReader> reader, Activity activity) {
            mImage = image;
            mFile = file;
                        ...
                        if(success){
                            if (hasInternetConnection()){
                                mactivity.runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        viewDialog.showDialog(mactivity,mFile,mContext,"camera");
                                    }
                                });
                            }
                            else {
...
//                }
        ...
        public static class ImageSaverBuilder {
            ...
            public synchronized ImageSaverBuilder setActivity(Activity activity) {
